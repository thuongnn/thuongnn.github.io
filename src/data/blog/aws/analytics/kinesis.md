...
- **Bản ghi Dữ liệu (Data Record)**:
  ...
  - **Số thứ tự (Sequence Number)**:
    ...
    - Kinesis gán số thứ tự khi nhà sản xuất dữ liệu gọi thao tác _PutRecord_ hoặc _PutRecords_ để thêm dữ liệu vào luồng.
...
- **Shard**:
  ...
  - Mỗi shard cung cấp một đơn vị dung lượng cố định. Nếu vượt quá giới hạn, hoặc do thông lượng dữ liệu hoặc số lượng bản ghi PUT, lệnh gọi dữ liệu sẽ bị từ chối với ngoại lệ _ProvisionedThroughputExceeded_.
  - Có thể xử lý bằng:
...
Dữ liệu có thể được thêm vào Kinesis Data Streams qua các thao tác API/SDK (_PutRecord_ và _PutRecords_), [Kinesis Producer Library (KPL)](http://docs.aws.amazon.com/kinesis/latest/dev/developing-producers-with-kpl.html) hoặc [Kinesis Agent](http://docs.aws.amazon.com/kinesis/latest/dev/writing-with-agents.html).
...
- **Kinesis Client Library (KCL)**:
  ...
  - Giúp dễ dàng xây dựng nhiều ứng dụng đọc từ cùng một luồng, _ví dụ: để thực hiện đếm, tổng hợp và lọc_.
